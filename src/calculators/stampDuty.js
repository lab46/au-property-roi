...
const STAMP_DUTY_RATES = {
  NSW: {
    ...
    firstHomeBuyer: { fullExemption: 800000, partialExemption: 1000000 },
  },
  VIC: {
    brackets: [
    ...
    firstHomeBuyer: { fullExemption: 600000, partialExemption: 750000 },
  },
  QLD: {
    brackets: [
    ...
    firstHomeBuyer: { fullExemption: 500000, partialExemption: null },
  },
  SA: {
    brackets: [
    ...
    firstHomeBuyer: { fullExemption: 650000, partialExemption: null },
  },
  WA: {
    brackets: [
    ...
    firstHomeBuyer: { fullExemption: 430000, partialExemption: 530000 },
  },
  TAS: {
    brackets: [
    ...
    firstHomeBuyer: { fullExemption: 600000, partialExemption: null },
  },
  NT: {
    rate: 0.0645,
    threshold: 525000,
  ...
  ACT: {
    ...
    firstHomeBuyer: { fullExemption: null, partialExemption: null },
  },
};
...
export function calculateStampDuty(propertyValue, state, isFirstHome = false) {
  ...
  if (!stateData) throw new Error(`Invalid state: ${state}`);
  ...
  if (isFirstHome && stateData.firstHomeBuyer) {
    ...
    if (fullExemption && propertyValue <= fullExemption) {
      return {
        state, propertyValue, isFirstHome, total: 0,
        concessionApplied: true, concessionType: 'Full Exemption',
    ...
    if (partialExemption && propertyValue <= partialExemption) {
      ...
      return {
        state, propertyValue, isFirstHome, total: fullDuty - discount,
        concessionApplied: true, concessionType: 'Partial Concession', savingsAmount: discount,
      };
    }
  }

  return {
    state, propertyValue, isFirstHome,
    total: calculateFullStampDuty(propertyValue, stateData),
    concessionApplied: false, concessionType: null, savingsAmount: 0,
  };
}

function calculateFullStampDuty(propertyValue, stateData) {
  if (stateData.rate !== undefined) {
    if (propertyValue <= stateData.threshold) return 0;
  ...
  for (const bracket of stateData.brackets) {
    if (propertyValue <= bracket.max) {
      ...
      return Math.round(bracket.base + (amountInBracket * bracket.rate / 100));
    }
  }
...
function calculatePartialConcession(propertyValue, fullExemption, partialExemption, fullDuty) {
  ...
  return Math.round(fullDuty * (1 - excess / range));
}
